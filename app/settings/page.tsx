...
export default async function SettingsPage() {
  ...
  return <>
    <header><p className="eyebrow">PROJECT SETTINGS</p><h1>项目设置</h1><p className="muted">选择左侧设置项，在右侧查看当前值并完成配置。</p></header>
    <SettingsNavigator items={navigationItems}>
      <section className="card settings-editor" aria-labelledby="workspace-settings-title">
        <div className="settings-editor-head"><span className="executor-icon"><FolderKanban size={18}/></span><div><p className="eyebrow">PROJECT</p><h2 id="workspace-settings-title">当前项目</h2><p>切换后，需求、运行记录和项目设置会自动使用该代码库对应的独立数据库。</p></div><span className="settings-current" title={paths.root}>{basename(paths.root)}</span></div>
      ...
      <section className="card settings-editor" aria-labelledby="flow-runtime-settings-title">
        <div className="settings-editor-head"><span className="executor-icon"><Bot size={18}/></span><div><p className="eyebrow">FLOW RUNTIME</p><h2 id="flow-runtime-settings-title">流程 Agent Runtime · 项目默认</h2><p>所有选择“跟随项目默认”的流程 Agent 会立即继承这里的执行器及对应参数。</p></div><span className="settings-current">{flowRuntimeSummary}</span></div>
        <form action={saveFlowAgentDefaultRuntimeAction} className="settings settings-editor-form">
          <div className="settings-panel-toolbar"><p className="muted">需要单独覆盖时，可进入对应 Agent Profile 配置。</p><Link className="button secondary" href="/agents">Agent 独立配置 <ArrowRight size={14}/></Link></div>
          <fieldset className="executor-settings"><legend>默认执行器</legend><div className="executor-options">
      ...
      <section className="card settings-editor" aria-labelledby="system-runtime-settings-title">
        ...
        <form action={saveAgentExecutorAction} className="settings settings-editor-form">
          ...
          <fieldset className="omp-settings"><legend>Oh My Pi 执行参数</legend><p className="muted">仅在选择 Oh My Pi 时生效。模型留空时使用 OMP 默认配置。</p><div className="fields"><label>模型<input name="ompModel" defaultValue={settings.ompModel} placeholder="例如 ollama/qwen3.6:35b、opus" spellCheck={false}/><small className="muted">支持 OMP 的模糊模型名或完整 provider/model。</small></label><label>思考强度<select name="ompThinking" defaultValue={settings.ompThinking}>{OMP_THINKING_LEVELS.map((level) => <option value={level} key={level}>{level === 'default' ? '跟随 OMP 默认值' : level}</option>)}</select></label></div><small>通过 <code>--approval-mode yolo</code> 自动批准工具；执行任务使用一次性会话，上下文对话会继续原会话。</small></fieldset>
