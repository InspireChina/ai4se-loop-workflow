...
export function AgentConfigurationEditor({ agentId, initialConfigurations }: {
  ...
  if (!selectedDocument) return <section className="card settings agent-section-card agent-configuration-editor">
    <div className="settings-section-head"><span className="executor-icon"><Workflow size={18}/></span><div><strong>Agent 行为配置</strong><p className="muted settings-description">当前 Agent 的 Prompt 配置集保存在 LoopWork 全局数据中；该 Agent 尚未迁移到 YAML 命令链。</p></div><span className={`badge ${selectedConfiguration.active ? 'green' : 'blue'}`}>{selectedConfiguration.active ? '当前生效' : '未启用'}</span></div>
    <div className="agent-config-toolbar"><div className="agent-config-choice-row"><label>配置集<select value={selectedConfiguration.configurationId} onChange={(event) => selectConfiguration(event.target.value)}>{sortedConfigurations.map((configuration) => <option value={configuration.configurationId} key={configuration.configurationId}>{configuration.name}{configuration.active ? ' · 生效中' : ''}</option>)}</select></label><form className="agent-config-inline-create" onSubmit={createConfiguration}><label>新配置<input value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="配置名称" maxLength={80}/></label><button className="button secondary" type="submit" disabled={busy || !newName.trim()}><CopyPlus size={14}/>复制</button></form></div><div className="form-actions compact">{!selectedConfiguration.active && <button className="button secondary" type="button" disabled={busy} onClick={() => void mutate({ action: 'activate', configurationId: selectedConfiguration.configurationId }, `“${selectedConfiguration.name}”已成为全局生效配置。`)}><Play size={14}/>启用</button>}{!selectedConfiguration.active && configurations.length > 1 && <button className="button danger-ghost" type="button" disabled={busy} onClick={() => { if (window.confirm(`删除配置“${selectedConfiguration.name}”？`)) void mutate({ action: 'delete', configurationId: selectedConfiguration.configurationId }, '配置已删除。'); }}><Trash2 size={14}/>删除</button>}</div></div>
    <p className="muted">可在 Prompt 页面编辑当前配置的全局 Prompt。配置集切换后，新 execution 会使用对应 Prompt；内置命令协议暂不可编辑。</p>
  ...
  return <section className="card settings agent-section-card agent-configuration-editor">
    <div className="settings-section-head"><span className="executor-icon"><Workflow size={18}/></span><div><strong>Agent 行为配置</strong><p className="muted settings-description">配置与运行参数都保存在 LoopWork 全局数据中，不随当前项目切换。</p></div><span className={`badge ${selectedConfiguration.active ? 'green' : 'blue'}`}>{selectedConfiguration.active ? '当前生效' : '未启用'}</span></div>
    <div className="agent-config-toolbar">
      ...
      <div className="form-actions compact">
        {!selectedConfiguration.active && <button className="button secondary" type="button" disabled={busy || dirty} onClick={() => void mutate({ action: 'activate', configurationId: selectedConfiguration.configurationId }, `“${selectedConfiguration.name}”已成为全局生效配置。`)}><Play size={14}/>启用</button>}
        {!selectedConfiguration.active && configurations.length > 1 && <button className="button danger-ghost" type="button" disabled={busy} onClick={() => { if (window.confirm(`删除配置“${selectedConfiguration.name}”？`)) void mutate({ action: 'delete', configurationId: selectedConfiguration.configurationId }, '配置已删除。'); }}><Trash2 size={14}/>删除</button>}
      </div>
    </div>
