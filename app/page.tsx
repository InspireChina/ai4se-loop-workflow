...
export default async function Home() {
  ...
  return <><header><div><p className="eyebrow">LOOP WORKBENCH</p><h1>工作台</h1><p className="muted">AI 自主推进；需要时补充设计决策或运行信息，并阅读最终结卡报告。</p></div></header>
    ...
    <section><h2>需要我处理</h2>{needsHuman.length === 0 ? <div className="empty">当前没有需要你处理的信息或结卡报告。</div> : needsHuman.map(({ task, lane, kind }) => <article className="attention" key={`${task.task_id}-${lane?.lane || kind}`}><AlertTriangle size={20}/><div><p className="eyebrow">{kind === 'closure' ? '待阅读结卡报告' : kind === 'runtime' ? `待补充运行信息 · ${lane!.lane === 'analysis' ? '交付分析' : '开发验证'} · ${agentLabel(lane!.current_agent)}` : lane ? `待回答关键决策 · 交付分析 · ${agentLabel(lane.current_agent)}` : `待回答需求澄清 · 需求级 · ${agentLabel(task.current_subagent)}`}</p><h3>{task.title}</h3><p>{terminologyText(lane?.blocked_reason || task.blocked_reason)}</p><small>{terminologyText(task.next_step)}</small></div><Link href={`/tasks/${task.task_id}`} className="button secondary">去处理 <ArrowRight size={14}/></Link></article>)}</section>
    <section><h2>正在推进</h2><div className="card table"><div className="row heading"><span>需求</span><span>状态</span><span>交付进度</span><span>下一步</span></div>{tasks.map((task) => {
      ...
      const label = requirementAnswers ? '等待需求澄清' : runtimeLane ? '等待运行信息' : answerLane ? '等待关键决策' : blockedLane ? `${blockedLane.lane === 'analysis' ? '交付分析' : '开发验证'}阻塞` : statusLabel(task.agile_status);
