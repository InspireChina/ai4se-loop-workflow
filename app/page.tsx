...
export default async function Home() {
  ...
  return <><header><div><p className="eyebrow">LOOP WORKBENCH</p><h1>工作台</h1><p className="muted">优先处理人工确认，再让 loop 按既有规则继续推进。</p></div></header>
    ...
    <section className="run-controls">
      {run?.active ? <form action={endLoopRunAction} className="run-banner"><LockKeyhole size={16}/><span>当前运行租约：{run.owner} · {run.leaseId}</span><small>{run.leaseUntil}</small><input type="hidden" name="leaseId" value={run.leaseId}/><button className="button secondary" type="submit">结束本轮</button></form> : <form action={startLoopRunAction} className="run-banner"><Route size={16}/><span>开始一轮 Loop，生成 run lease 和 dispatch 文件</span><button className="button" type="submit">开始 Loop</button></form>}
    </section>
