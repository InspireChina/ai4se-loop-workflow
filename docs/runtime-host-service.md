...
桌面首屏、普通业务能力和 Admin 调度是不同阶段。当前默认采用标准模式：Electron 单实例与管理库代次令牌负责防止同一数据目录出现两个有效写入者；新启动的桌面 root 可以直接 fencing 旧租约，普通业务宿主和 UI 不再等待旧 root、旧 Windows Job 或 Admin 清理完成。当前安装包的 runtime 只读取构建绑定后直接运行，不再为日常启动执行全量文件哈希、内容寻址复制或旧版本交接。Admin 的配置发现、RepairCase、Action 和 Follow-up 全部在后台继续，失败只进入诊断，不阻塞用户打开界面或执行普通业务。

进程身份观测仍使用三态语义：明确 PID 死亡或读到不同启动标记立即失效；读到相同标记继续；OS 查询临时失败为 unknown。标准模式下，普通 UI、业务宿主和 Agent CLI 不把 Windows Job 查询或完整后代退出证明作为同步准入条件；它们依赖当前 root、host 与 business generation 的令牌校验。严格 containment 仍可用于隔离验收和高风险维护流程，但不再是桌面日常启动条件。
