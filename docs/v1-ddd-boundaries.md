...
`Question` 的正文、推荐答案、用户答复都以 SQLite 为事实来源；Agent 不再写 question Markdown。`Approval` 表达 analysis 的 `pending/confirmed` 与 review 的 `pending/approved`，当前由全部问题完成回答后自动确认。
...
| `questions` | Question 事实来源 | 存储结构化问题、推荐答案和用户答复。 |
...
数据库中的 `stories`、`artifacts`、`approvals` 是让 UI 可以查询、筛选、关联和呈现的本地投影；`questions` 是线上化后的事实来源。
