...
负责按项目隔离各流程 Agent 的实际 Role Prompt、Durable Memory、daily observation、版本历史和 Prompt Canary。

- 模型：`AgentProfile`、`PromptVersion`、`MemoryRevision`、`EvolutionObservation`、`EvolutionRun`
...
内置 Prompt 只负责首次 seed。SQLite 保存版本与证据，本地 Runtime Workspace 物化当前实际文件；目标代码库不拥有这套配置。Evolution Evaluator 只能提出结构化观察，不能写流程状态、修改 Core Contract 或自行决定提升。Prompt candidate 必须由匹配 candidate ID 的真实 execution attempt 验证，任何失败立即回滚；整个演化链路不能阻塞主 Loop。
