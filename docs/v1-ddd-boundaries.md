...
| 运行信息请求 | 当前 Agent 为继续执行所需、无法从现有上下文推导的非敏感信息；不是产品决策或 Approval。 |
...
代码槽繁忙不是产品澄清。需要写代码的步骤进入内部等待队列，释放后自动继续。开发实现 Agent 直接使用当前主干工作区，可以只提交本轮相关代码，也可以不提交；V1 接受已有改动与 Agent 修改同文件时难以完全区分归属的风险，不由 Runner 创建 checkpoint 或代理提交。
...
12. 每轮最多派发一个需要独占浏览器的 Agent 执行。
13. 同一个 execution attempt 的 Agent Commit（如有）、验证和 Agent Result 必须幂等记录。
14. execution attempt 必须记录实际使用的 Prompt/Memory 版本和哈希。
15. Prompt 自动提升必须满足证据阈值并通过三次真实 Canary；任一次失败立即回滚。
16. 主 Runner 的 finally 只能持久化维护任务，不能同步修改代码或等待 Maintenance Agent。
17. runtime event 在持久化前必须脱敏，并保留 run/execution correlation、severity 和稳定异常 fingerprint。
18. 软件修复候选只能在独立 worktree 生成；变更预算、保护路径、test/build 和 clean baseline 缺一不可。
