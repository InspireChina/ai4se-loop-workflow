...
Feedback Agent 有两个模式：Triage 为批次内每条评论分别返回影响、`targetStage` 和完成标准；Verify 在目标 Agent 完成修改、Test/Harness 产生证据后，逐条检查评论是否真正得到满足。Harness 将 `context / repro / plan / analysis / dev / test / review` 映射到唯一负责 Agent，并把整批决策合并成一次最早回退。较晚阶段评论保持待处理，等正常 Loop 到达对应阶段时注入目标 Agent，不再跳转。若 `context / repro / plan` 回退重建交付单元，原单元评论先等待重新绑定，新拆分完成后只重新定位。Review Agent 只生成更新后报告，不对已分流反馈再次发起回退。任何未验证评论都会阻止结卡；最终只有 Harness 可以将反馈标记为 `resolved`。

```mermaid
flowchart LR
    comment["冻结当前评论批次"] --> triage["Feedback Agent · 批量 Triage"]
    triage --> frontier["Harness · 一次最早回退"]
