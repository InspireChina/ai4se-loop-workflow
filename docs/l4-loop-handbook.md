...
需求梳理 Agent 是原始输入与 L4 Development 正式起点“已对齐需求规格”之间的桥梁。它依据当前项目能够提供的证据，区分当前实际业务状态、当前应有的业务语义和本次目标状态，明确三者之间的变化，并分析对业务结果有实质意义的影响。不同项目的知识维护成熟度并不一致；当业务知识主要存在于代码中时，Agent 应深入代码、测试和真实入口还原当前行为，不能因此降低调查深度。但调查深度与交付表达层级必须分离：文件、函数、接口、字段和测试是获取与定位知识的手段，提交到需求上下文的结论必须提升为参与者、触发条件、业务规则、状态变化和可观察结果。代码、测试和运行结果通常只能证明当前实现，不能自动定义正确业务规则或覆盖用户目标。

基础角色只规定认识论、业务边界和完成标准，不预设固定行业检查清单。Agent 自主选择与当前项目匹配的调查路径；发现关联影响不代表自动将其纳入范围，必须区分需要同步改变、必须保持不变、需要业务决策和留给后续技术分析的影响。纯技术影响在本阶段只作为后续需要调查的工程约束或风险，不形成文件计划、代码设计、技术选型或候选解法。草稿中的稳定 key 表示稳定语义身份，错误结论必须显式排除或取代并保留修订历史。需求梳理 Agent 不拆分交付单元，也不设计实现方案。
...
交付单元冻结后，交付分析 Agent 不直接跳到文件计划，而是根据项目实际在调查和推理中完整走过一次交付：先从参与者和用户可观察结果理解业务承诺，再从项目真实结构检查它需要经过的系统边界。Do It Twice 是思考方法，不是要求逐项持久化的文档模板。代码、测试、配置、数据模型和运行观察用于证明当前项目事实，但不自动覆盖上游业务承诺。

思考过程中应显式发现需求文字未覆盖的实际影响，以及所有可能导致交付结果分叉的选择。选择是否重要，不取决于它属于界面、数据还是代码，也不取决于修改是否容易，而取决于不同选择是否会产生不同的业务语义、用户可观察结果、信息层级、交互路径、公共契约、数据语义、兼容承诺、重大工程后果或独立验证预期。每个真实影响最终处理为改变、保持、排除或待决策。决策权留给有权承担结果的一方：上游已经承诺的直接继承，具备决定权的项目事实能够唯一确定的依据事实关闭，对交付结果等价的专业工程选择由 Agent 自主关闭；存在多个合理且结果不同的选择、又没有权威依据时交给用户决定。容易修改、技术上可逆或 Agent 能够实现，都不能代替产品决定权。

交付分析的结果不是逐文件开发计划，也不是完整推演档案，而是一份轻量、版本化的冻结交付契约：分析摘要、实际影响及处理方式、关键业务和技术决策、实现方向、保护约束与验证关注点。它是 Dev 与 Test 的共同上游事实，而不是二者之间的交接。完成时，两个合理的 Dev Agent 不能在都声称遵守契约的情况下交付出实质不同的用户可观察结果，两个独立的 Test Agent 也不能从契约推导出不同的正确预期；剩余自由度只能是结果等价的实现与验证选择。
...
Review Agent 根据原始需求、所有已收敛交付规格、用户澄清、当前仓库状态、Test Evidence 和执行事实生成结卡报告。报告至少包括：
...
Feedback Agent 有两个模式：Triage 把批次评论按共同验收目标分成工作组；Verify 在追加工作产生新文档或 Test Evidence 后，逐条检查评论是否真正得到满足。直接回复、历史说明和长期建议不创建交付单元；Bug 先复现；需要工程实施的 Bug、行为修订、范围新增和技术调整全部先交给交付规划 Agent，由其形成一个或多个具备稳定身份、参与者、触发条件、可观察结果、验收语义和来源关联的完整追加单元；报告修订生成新版本。Application 不接受评论驱动的阶段跳转或回退。旧文档、旧交付规格和旧交付单元保持不变，新工程工作只追加到末尾并按正常 Loop 前进。任何未验证评论或活动反馈批次都会阻止结卡，验证未通过的评论进入一个新批次。
