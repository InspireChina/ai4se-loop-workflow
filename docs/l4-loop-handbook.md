...
交付规划不是把需求改写成一组标题。它冻结需求梳理阶段已经收敛的业务变化上下文，将必须同步改变的影响、必须保持的约束、交给后续分析的技术后果和需求级验收语义，显式映射到有序交付单元。每个单元保留稳定身份、业务参与者、触发条件、可观察结果、独立验收语义和自然依赖，并作为 Analyst 的正式输入契约。
...
Feedback Agent 有两个模式：Triage 把批次评论按共同验收目标分成工作组；Verify 在追加工作产生新文档或 Test Evidence 后，逐条检查评论是否真正得到满足。直接回复、历史说明和长期建议不创建交付单元；Bug 先复现，行为修订和技术调整各追加一个单元，范围新增先由交付规划 Agent 拆分，报告修订生成新版本。Application 不接受评论驱动的阶段跳转或回退。旧文档、旧 Slice Spec 和旧交付单元保持不变，新工程工作只追加到末尾并按正常 Loop 前进。任何未验证评论或活动反馈批次都会阻止结卡，验证未通过的评论进入一个新批次。
