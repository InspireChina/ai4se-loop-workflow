...
因此，我们建议使用单独的测试 Agent。Test 以交付分析冻结的业务承诺、实际影响、保护约束和验证关注点建立 Oracle，以当前仓库和真实环境作为验证对象；它不读取或依赖 Dev Agent 的完成声明、自检、开发记录、变更说明、Commit message 或恢复声明，而是独立生成测试计划，再通过前端或其他用户入口模拟 QA 进行测试。
