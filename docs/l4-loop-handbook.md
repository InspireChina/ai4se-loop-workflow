...
flowchart LR
    ...
    lanes --> clarify["Analysis Lane · 单元级澄清"]
    lanes --> dev["Delivery Lane · 开发"]
    clarify --> dev
    dev --> test["Delivery Lane · 测试"]
    test --> passed{"测试通过?"}
    passed -->|"否"| dev
...
3. 在 Happy Path 中，人只需要回答无法推导的需求级产品歧义、单元级产品决策或重大技术决策，并参与统一验收；需求级回答交回需求梳理 Agent，单元级回答交回对应 Analyst。

交付拆分后，Analysis Lane 可以连续分析后续交付单元，Delivery Lane 在后方串行执行 `Dev(N) → Test(N)`。两条 Lane 的状态、阻塞和恢复相互独立，但 Dev 永远不能超过已经 resolved 的分析进度。任一 Lane 完成后立即重新调度；进入 Review 前，两条 Lane 必须全部汇合且没有开放反馈。
...
在最小交付单元下，Analyst 先列出关键决策树。它既包括影响用户可观察行为、数据语义、范围、权限、兼容性或失败处理的产品决策，也包括架构边界、公开接口、持久化与迁移、跨模块依赖、安全、性能、部署运维以及代价高且难回退的重大技术决策。每个决策要么由代码、用户答复或明确项目文档证明，要么必须向人提问；推荐方案不能替代用户作答。命名、局部代码组织、既有组件用法和容易回退的等价实现仍由 Agent 自主决定。

这里不能在拆分交付单元前进行对齐。拆分前的范围尚未确定，因此需要先完成拆分，再进行业务和技术对齐。

#### 抽查开发计划和 AC

通常，在关键决策树对齐后，Agent 已经可以自行实现整个功能。系统仍然要求其在开发前写好计划，便于团队随时审查。团队可以偶尔抽查开发计划，判断 Prompt 是否需要更新，而不是逐任务审批。
...
LoopWork 中，每次 Agent 调用都对应一个持久化的 `execution_attempt`。系统在调用前保存输入、Prompt 版本和起始 Git HEAD；Agent Result 和状态应用分别保存 Receipt。Execution 不使用超时租约，Loop Run 通过 Runner 进程存活和短周期心跳识别主动停止、崩溃与突然关机。
...
Review Agent 根据原始需求、所有 resolved Slice Spec、用户澄清、实际 Commit、Test Evidence 和执行事实生成结卡报告。报告至少包括：
...
Feedback Agent 有两个模式：Triage 为批次内每条评论分别返回影响、`targetStage` 和完成标准；Verify 在目标 Agent 完成修改并产生新的文档或 Test Evidence 后，逐条检查评论是否真正得到满足。Application 将 `context / repro / plan / analysis / dev / test / review` 映射到唯一负责 Agent，并把有效决策合并成一次最早回退；本轮遗漏或无效的评论继续留在队列，下轮再次处理，不让整个批次失败。较晚阶段评论保持待处理，等正常 Loop 到达对应阶段时注入目标 Agent。若 `context / repro / plan` 回退重建交付单元，原单元评论先等待重新绑定，新拆分完成后只重新定位。Review Agent 只生成更新后报告，不对已分流反馈再次发起回退。任何未验证评论都会阻止结卡，Feedback Agent 的验证结论由 Application 持久化为 `resolved` 或 `reopened`。

```mermaid
flowchart LR
    comment["冻结当前评论批次"] --> triage["Feedback Agent · 批量 Triage"]
    triage --> frontier["Application · 一次最早回退"]
    frontier --> target["按正常 Loop 到达目标 Agent"]
    target --> evidence["文档 / Test Evidence"]
    evidence --> verify["Feedback Agent · Verify"]
    verify --> result{"满足评论?"}
    result -->|"是"| resolved["Application 持久化 resolved"]
...
LoopWork 已经具备这套 Infra 的早期形态，包括确定性编排、Agent Profile、版本化 Slice Spec、execution attempt、Test Evidence、Recovery Item、rewind、日志、Trace 和 Memory。下一步可以将固定流程抽象为可配置的 Workflow Profile，并通过 Agent、Skill、Policy 和 Adapter 支持不同项目及 L4 Profile。
