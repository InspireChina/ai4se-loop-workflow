...
Review Agent 是只报告角色：信息充分时只能返回完整 artifact 和 `verdict=report_ready`，不能提出产品问题、请求审批或回退；确实缺少报告所需的非敏感运行信息时仍使用 `runtimeInputs`。
