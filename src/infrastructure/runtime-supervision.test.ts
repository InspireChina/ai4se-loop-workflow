...
test('only the current external activating lease clears business update state; dispatch stays held until external health completion', async () => {
  ...
  const request={updateId:randomUUID(),caseId:'original-case',
    ...
    candidate:{root:process.cwd(),sourceId:'c'.repeat(64),artifactId:'d'.repeat(64),version:releaseVersion}};
  h.store.beginRuntimeUpdate(request);
  h.db.prepare("UPDATE loop_lifecycle_state SET mode='update-silence',update_target_version=? WHERE singleton=1").run(releaseVersion);
  const host=createManagedLoopRunLifecycle({ownerId:h.ownerId,adapter:'cli',installedVersion:releaseVersion,dependencies:h.dependencies,
