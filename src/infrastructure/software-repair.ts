...
const protectedPath = /^(?:\.git(?:\/|$)|\.env(?:\.|$)|data\/|node_modules\/|migrations\/|app-migrations\/|package(?:-lock)?\.json$|(?:pnpm-lock\.yaml|yarn\.lock)$|tsconfig\.json$|next\.config\.[^/]+$|scripts\/loop\/(?:loop-agent\.mjs|maintenance-runner\.ts|submit-agent-result\.mjs)$|src\/application\/(?:internal-agent-command-drafts|software-maintenance|runtime-events)\.ts$|src\/infrastructure\/(?:agent-result-channel|software-repair|maintenance-runner)\.ts$)/i;
