...
test('allows flow agents to run for four hours by default', () => {
  ...
  assert.equal(DEFAULT_AGENT_EXECUTOR_STARTUP_TIMEOUT_MS, 20 * 60 * 1000);
  ...
  assert.deepEqual(resolveAgentExecutionLimits({}), {
    ...
    startupTimeoutMs: DEFAULT_AGENT_EXECUTOR_STARTUP_TIMEOUT_MS,
...
test('uses one validated timeout source for agent execution', () => {
  const env = {
    AGENT_EXECUTOR_TIMEOUT_MS: '18000000',
    AGENT_EXECUTOR_STARTUP_TIMEOUT_MS: '90000',
    AGENT_EXECUTOR_IDLE_TIMEOUT_MS: '1200000',
  };
  assert.deepEqual(resolveAgentExecutionLimits(env), {
    maxRuntimeMs: 18_000_000,
  ...
  assert.equal(resolveAgentExecutionLimits({ CURSOR_AGENT_STARTUP_TIMEOUT_MS: '60000' }).startupTimeoutMs, 60_000);
});
