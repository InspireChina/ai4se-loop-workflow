...
test('Review generic chain compiles closure gaps into forward delivery units', async () => {
  ...
  const db = await databaseConnection();
  adoptNativeWorkflowInDb(db, fixture.taskId);
  const nativeDelegation = (await inspectTaskDispatch(fixture.taskId))
    .find((item) => item.agent === 'review-agent' && item.pipeline === 'review');
  assert.ok(nativeDelegation);
  const started = await begin({ ...fixture.delegation, ...nativeDelegation! }, `${fixture.taskId}-gap`, fixture.resources);
  await command(started.executionId, started.token, ['status']);
  await command(started.executionId, started.token, ['phase', 'complete']);
