...
test('initializes one project-owned Prompt from the system template without overwriting project edits', async () => {
  ...
  assert.equal(original.profile.prompt_seed_revision, 8);
  assert.equal(original.currentPrompt.version, 1);
  assert.equal(original.currentPrompt.template_version, 8);
  ...
  assert.equal(upgradedSystemSeed.profile.prompt_seed_revision, 8);
  assert.equal(upgradedSystemSeed.currentPrompt.version, 2);
  assert.equal(upgradedSystemSeed.currentPrompt.template_version, 8);
  ...
  assert.equal(resetBaseline.currentPrompt.template_version, 8);
  ...
  assert.equal(runtime.promptTemplateVersion, 8);
  ...
  assert.equal(reset.currentPrompt.template_version, 8);
  assert.equal(reset.currentPrompt.source, 'system');
  assert.equal(reset.currentPrompt.reason, '用户重置为系统模板 V8');
