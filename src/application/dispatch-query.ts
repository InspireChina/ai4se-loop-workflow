...
import { RESOURCE_DEFINITIONS, resourcesForAgent, resourcesRequiringClaims, type ResourceKey } from '../domain/resource';
...
import type { FeedbackItemContext } from './work-item-feedback';
import type { DelegationEnvelope, Task } from './tasks';
import type { WorkflowItemRow } from './work-items';

export type DispatchQueryPorts = {
  activeResourceClaimInDb: typeof import('./resource-claims').activeResourceClaimInDb;
  resourceIdentityInDb: typeof import('./resource-claims').resourceIdentityInDb;
  resourceScopeInDb: typeof import('./resource-claims').resourceScopeInDb;
  taskContextChatTurnIsRunning: typeof import('./task-context-chat').taskContextChatTurnIsRunning;
  agentConcurrencyInDb: typeof import('./project-settings').agentConcurrencyInDb;
  requirementDependencyGateOpenInDb: typeof import('./task-dependencies').requirementDependencyGateOpenInDb;
  readyWorkflowItemsForTaskInDb: typeof import('./work-items').readyWorkflowItemsForTaskInDb;
  nativeWorkflowEndedInDb: typeof import('./work-item-controls').nativeWorkflowEndedInDb;
  workflowBlockedInDb: typeof import('./work-item-controls').workflowBlockedInDb;
  executionProcessBarrierInDb: typeof import('./execution-processes').executionProcessBarrierInDb;
  repairResourceClaimInDb: typeof import('./repair-resources').repairResourceClaimInDb;
  supportsResume: (agent: string) => boolean;
  refreshWorkflowForDispatchInDb?: (db: Database.Database, task: Task) => void;
};

type Db = Database.Database;

export const dispatchTaskSelect = `
...
/** Shared selection has only pure runtime imports. All persistence,
 * configuration and reconciliation are explicit ports. Independent readers
 * use inspectDispatchInDb with read-only ports; ordinary admission retains
 * its existing business adapter. */
export function createDispatchQuery(ports: DispatchQueryPorts) {
  const { activeResourceClaimInDb, resourceIdentityInDb, resourceScopeInDb,
    ...
    executionProcessBarrierInDb, repairResourceClaimInDb, supportsResume } = ports;
  function refreshWorkflowForDispatchInDb(db: Db, task: Task) {
    if (!ports.refreshWorkflowForDispatchInDb) throw new Error('派发 admission 需要显式业务协调适配器；只读能力应使用 inspection');
    ports.refreshWorkflowForDispatchInDb(db, task);
  }
  ...
  function workItemLine(task: Task, item: WorkflowItemRow): Delegation | null {
    ...
    const pipeline = item.resume_pending && !item.pipeline.startsWith('feedback-') && supportsResume(item.agent)
  ...
  function schedulingResourceClaims(db: Db, releaseStale = true) {
    ...
    const rows = db.prepare(`SELECT resource_key, owner_task_id FROM resource_claims
      UNION SELECT resource_key,owner_task_id FROM execution_process_barriers barrier
        JOIN execution_processes process USING(allocation_id) WHERE process.status <> 'exited'`)
  ...
  function resourcesAvailable(
    ...
    return resourcesRequiringClaims(delegation.resources).every((resourceKey) => {
      if (repairResourceClaimInDb(db, resourceKey, resourceScopeInDb(db, resourceKey, delegation.taskId))) return false;
      if (executionProcessBarrierInDb(db, resourceKey, resourceScopeInDb(db, resourceKey, delegation.taskId))) return false;
  ...
  function planDispatchInDb(db: Db, options: { refresh?: boolean } = {}): DelegationEnvelope[] {
  ...
  function inspectDispatchInDb(db: Db): DelegationEnvelope[] {
    return planDispatchInDb(db, { refresh: false });
  }

  function projectRequirementWorkInDb(db: Db, taskId: string): Delegation[] {
  ...
  const dispatchProjectionSupport = { dispatchTaskSelect,
  ...
  return { planDispatchInDb, inspectDispatchInDb, projectRequirementWorkInDb, dispatchProjectionSupport };
}
