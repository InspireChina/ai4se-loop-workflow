...
export function agentCommandWorkKey(agent: string, pipeline: string, taskId: string, storyIndex: number | null) {
...
export function agentCommandPrompt(appRoot: string, agent: string, pipeline: string) {
  ...
  return [
    ...
    'status 会列出当前事实、约束、范围和问题的稳定 key。修改既有语义时必须复用原 key 进行覆盖或删除；只有新增语义才创建新 key，禁止通过换名制造重复条目。问题的 decision key 跨轮次绝对不可改名。',
