...
test('exposes Business Analysis separately from the engineering delivery pipelines', () => {
  ...
  assert.deepEqual(REQUIREMENT_PIPELINES.map((pipeline) => pipeline.label), ['Business Analysis', 'Develop', 'Bug Fix']);
  assert.deepEqual(
...
test('accepts only pipelines available from requirement creation', () => {
  ...
  assert.throws(() => requirementPipeline('tech'), /只能选择 Business Analysis、Develop 或 Bug Fix/);
  assert.throws(() => requirementPipeline(null), /只能选择 Business Analysis、Develop 或 Bug Fix/);
});
