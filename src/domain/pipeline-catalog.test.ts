...
test('exposes standalone, end-to-end, and engineering delivery pipelines', () => {
  assert.deepEqual(REQUIREMENT_PIPELINES.map((pipeline) => pipeline.id), ['business-analysis', 'end-to-end', 'feature', 'bug']);
  assert.deepEqual(REQUIREMENT_PIPELINES.map((pipeline) => pipeline.label), ['Business Analysis', 'End to End', 'Develop', 'Bug Fix']);
  assert.deepEqual(
...
test('accepts only pipelines available from requirement creation', () => {
  ...
  assert.throws(() => requirementPipeline('tech'), /只能选择 Business Analysis、End to End、Develop 或 Bug Fix/);
  assert.throws(() => requirementPipeline(null), /只能选择 Business Analysis、End to End、Develop 或 Bug Fix/);
});
