...
test('ships rigorous V1 seed prompts for every flow Agent', () => {
  ...
  assert.match(AGENT_PROFILE_DEFINITIONS['backlog-agent'].prompt, /没有独立业务文档时，应当深入代码和真实入口还原当前行为/);
  assert.match(AGENT_PROFILE_DEFINITIONS['backlog-agent'].prompt, /调查可以深入到文件、函数、接口、字段和测试，但提交前必须把发现提升为参与者、触发条件、业务规则、状态变化和可观察结果/);
