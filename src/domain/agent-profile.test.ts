...
test('ships rigorous versioned seed prompts for every flow Agent', () => {
  assert.equal(AGENT_PROMPT_SEED_REVISION, 32);
  ...
  assert.match(AGENT_PROFILE_DEFINITIONS['test-agent'].prompt, /Oracle 来自冻结的业务承诺、影响范围、保护约束与验证关注点/);
  assert.match(AGENT_PROFILE_DEFINITIONS['test-agent'].prompt, /不得读取或依赖 Dev Agent 的结果文档、自述、自检、变更说明或恢复声明/);
  assert.match(AGENT_PROFILE_DEFINITIONS['test-agent'].prompt, /Commit message 或恢复声明都不能作为通过依据/);
