...
test('ships rigorous V1 seed prompts for every flow Agent', () => {
  assert.equal(AGENT_PROMPT_SEED_REVISION, 1);
  ...
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /是思考方法，不是文档模板/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /当前交付单元是不可重新解释的业务承诺/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /delivery-analysis status/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /不存在关键决策时可以不创建决策/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /原 decision key/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /保留不改变交付语义的实现与测试自主空间/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /共同上游事实，不是两个 Agent 之间的信息交接/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /需求文字只提到一个位置而项目事实显示三个位置/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /建表还是新增字段/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /不得修改工作区文件或提交代码/);
  assert.match(AGENT_PROFILE_DEFINITIONS['analyst-agent'].prompt, /delivery-analysis complete/);
