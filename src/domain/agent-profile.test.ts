...
test('ships rigorous V15 seed prompts for every flow Agent', () => {
  ...
  assert.doesNotMatch(AGENT_PROFILE_DEFINITIONS['backlog-agent'].prompt, /request-clarification|requirement-context complete/);
  assert.match(AGENT_PROFILE_DEFINITIONS['backlog-agent'].prompt, /第一条草稿命令必须是 status/);
  assert.match(AGENT_PROFILE_DEFINITIONS['backlog-agent'].prompt, /使用 phase complete 推进/);
  ...
  assert.match(AGENT_PROFILE_DEFINITIONS['story-splitter-agent'].prompt, /第一条草稿命令必须是 status/);
  ...
  assert.match(AGENT_PROFILE_DEFINITIONS['story-splitter-agent'].prompt, /phase rewind/);
  assert.match(AGENT_PROFILE_DEFINITIONS['story-splitter-agent'].prompt, /最后一个 phase complete/);
  assert.doesNotMatch(AGENT_PROFILE_DEFINITIONS['story-splitter-agent'].prompt, /delivery-plan (?:status|complete|validate)/);
