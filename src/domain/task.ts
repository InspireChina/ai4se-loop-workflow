...
export const RUN_STATES = ['runnable', 'waiting_for_answers', 'system_blocked', 'idle'] as const;
...
export function occupiesCodeSlot(task: TaskState) {
  return task.agile_status === 'in dev' || (task.agile_status === 'blocked' && task.resume_status === 'in dev');
}
