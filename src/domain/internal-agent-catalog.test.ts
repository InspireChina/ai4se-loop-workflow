...
test('internal Agent catalog is complete, uniquely identified and read-only', () => {
  assert.deepEqual(INTERNAL_AGENT_DEFINITIONS.map(agent => agent.id), [
    'system-assistance-agent',
    'admin-repair-agent',
    'independent-repair-verification-agent',
  ...
  assert.ok(INTERNAL_AGENT_DEFINITIONS.every(agent => agent.label.trim() && agent.description.trim()
    && agent.runtimeIdentity.trim() && agent.trigger.trim() && agent.authority.trim()));
  assert.deepEqual(INTERNAL_AGENT_DEFINITIONS.map(agent => agent.id)
