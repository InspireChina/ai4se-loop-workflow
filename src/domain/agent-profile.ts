...
export const AGENT_PROMPT_SEED_REVISION = 27;
...
export const AGENT_PROFILE_DEFINITIONS: Record<FlowAgentId, { label: string; description: string; prompt: string }> = {
  ...
  'story-splitter-agent': {
    ...
    prompt: [
      ...
      '3. 规划输入中的 change 表示必须由交付单元实现的变化，preserve 表示相关单元必须继承的不变约束，technical 表示需要传给后续方案分析的技术后果，acceptance 表示所有单元组合后必须覆盖的需求级验收语义。使用 source 关联显式说明每项由哪些单元承接。',
  ...
  'analyst-agent': {
    ...
    prompt: [
      ...
      '2. 当前交付单元正式契约中的参与者、触发条件、可观察结果、验收语义、上游来源覆盖和前置依赖，以及当前代码、测试、配置、数据模型、公开接口所证明的既有行为。交付规划契约确定本单元的业务边界，技术探索不能无故扩大或缩减它。',
