...
这个系统不是多 Agent 自由协作，也不是每个阶段都等待人工审批的工作流。整体流程由确定性的 App 和 Runner 调度，AI 负责理解、分析、实现、验证、审查和回流；人通常只在两个固定业务节点参与：

1. 开发前，回答 AI 无法从代码、文档和已有决策中推导出的产品歧义。
2. 开发后，阅读 AI 生成的完整结卡报告，确认已经知晓交付结果，然后关闭任务。

此外，当目标仓库的 Git hook 要求无法从仓库和需求上下文推导的提交元数据时，Runner 会进入 `waiting_for_git_input`，只询问完成当前提交所缺的信息。这是运行恢复，不是产品澄清或 Approval；回答后从已保存的 Git 阶段继续，不重新运行开发 Agent。
...
| Dev Agent | 走查当前开发单元，补齐缺失实现并提供实现声明 | 否 |
...
1. Runner 在调用 Agent 前保存 delegation、Prompt 哈希、输入快照、基准 Commit 和租约。
2. 收到合法结构化结果后立即持久化，随后才允许提交代码、执行 Harness 和应用状态变化。
3. 代码 Commit、Harness run 和 Application result 各自生成幂等收据。
4. 进程若在 Agent 输出后中断，下一轮从保存的输出继续，不再次调用 Agent。
5. 进程若在输出前失去租约，attempt 进入可重试失败；同一输入最多自动尝试三次，之后才进入 `system_blocked`。

`waiting_for_answers`、`waiting_for_git_input` 与 `system_blocked` 是三件不同的事：第一种是产品澄清，回答后回到 Analyst；第二种是仓库提交策略要求补充运行信息，回答后只重试 checkpoint 或交付 commit；第三种代表无法通过补充信息安全恢复的执行环境、结构化协议或恢复机制异常。
...
选择 Codex 时，可为当前项目从 GPT-5.6 Sol、Terra 和 Luna 三档模型中选择，并单独配置思考强度。Runner 使用 `--model` 和 `--config model_reasoning_effort=...` 传递显式覆盖。开发实现 Agent 启动前若已有未提交改动，Runner 会先创建独立 checkpoint commit，再将当前交付单元的实现提交为另一个 commit；敏感文件仍会阻止自动提交。若仓库提交策略拒绝标题，Runner 会保留工作区和 execution output，等待用户提供合规标题；可选的仓库级模板支持 `{taskId}`、`{externalId}`、`{unit}`、`{type}`、`{description}`、`{operation}` 和 `{title}` 变量。
...
- `system_blocked`、产品澄清和 Git 提交信息恢复分离；提交恢复使用 commit 哈希与 execution receipt 追踪，不强迫目标仓库标题包含内部 Requirement ID。
...
- Review Agent 只生成版本化结卡报告，`ready_to_close` 仅等待阅读确认。
