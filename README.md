...
Loop 生命周期与推进流程只由 Web App 和内部 Runner 管理。每次只执行一个 Agent，Runner 注入完整需求上下文并解析最终结构化 JSON；Application 自动保存文档、确认事项和结果，再计算下一步。Agent 不调用 `loopctl`，也不负责判断整体流程。单个 Agent 可以使用辅助 subagent 收集当前执行步骤的上下文，但不能调度其他流程 Agent。
...
src/application/     需求、确认事项、blocked 等用例
