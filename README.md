...
UI 运行面板可以点击“开始运行”，创建本地 Run 并逐条执行 delegation。Run 使用 PID 防止重复启动，不限制持续运行时长。项目设置中可以选择 Cursor、Codex 或 Claude，默认使用 Cursor：
...
Run 生命周期与 Pipeline 派发只由 Web App 和内部 Runner 管理。每次只派发一个 Agent，Runner 注入完整 Task 上下文并解析最终结构化 JSON；Application 自动保存文档、问题和结果并推进状态，然后重新计算下一步。Agent 不调用 `loopctl`，也不理解流程游标。单个 agent 可以使用辅助 subagent 收集当前 delegation 的上下文，但不能调度其他 pipeline agent。
