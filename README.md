...
2. Agent 完成工作后调用 execution 专属的 `submit-agent-result` 命令提交结构化 Result Receipt；CLI 同步执行完整领域 Schema 和静态角色契约校验，失败时保留输入供 Agent 当场修正重提。Runner 收到后再防御性复验并持久化，随后才允许执行 Harness 和应用状态变化。最终文本 JSON 只作为旧执行器兼容 fallback。
...
选择 Codex 时，可为当前项目从 GPT-5.6 Sol、Terra 和 Luna 三档模型中选择，并单独配置思考强度。选择 Claude 时，可以填写 Claude CLI 支持的模型别名或完整模型 ID；留空则跟随 CLI 默认值。Runner 分别使用 `--model` 和 Codex 的 `--config model_reasoning_effort=...` 传递显式覆盖。开发实现 Agent 直接在当前主干工作区走查和修改；它可以按目标仓库规则只提交本轮相关代码，也可以在现有实现已满足规格或不适合提交时不创建 commit。Runner 不创建 checkpoint、不代理提交，也不把 commit 作为完成条件，只记录 Agent 执行前后的 Git HEAD 并运行 Harness。
