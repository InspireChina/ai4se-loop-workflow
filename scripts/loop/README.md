...
流程 Agent 需要通过 `--*-file` 传递长文本时，只能写入 Runner 注入的 `$LOOP_AGENT_TMP_DIR`。该路径位于目标工作区 `.tmp/loop-<run-id>/agent-<execution-id>`；同一 Loop 内各 Agent 相互隔离，但文件保留到整个 Loop Run 结束，随后由 Runner 最外层 `finally` 统一清理。CLI 会拒绝读取该目录之外的 `--*-file`，临时材料不得写入源码目录或提交到 Git。
