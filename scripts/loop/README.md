...
Loop 生命周期、推进步骤和结束运行只由 Web App 与内部 Runner 管理。交付拆分后，每个任务最多同时运行一个 Analysis Agent 和一个 Delivery Agent；Analysis 独立向前，Delivery 串行执行 Dev/Test。任一 Lane 完成都会立即触发重新调度，不存在整批等待屏障。Runner 注入该任务的完整上下文并接收结构化 JSON；正常流程 Agent 不调用本 CLI。任务详情页的上下文 Chat 是独立通道：它通过读取命令获取最新事实，不参与状态推进。Dev/Test 正在运行时，Chat 使用只读 Prompt；工作区空闲时，Chat 使用轻量修改 Prompt，可完成不改变业务语义的 UI / wording 小改动并提交自己的代码。Chat 不排队；轻量修改执行期间只暂缓启动新的 Delivery Agent。
