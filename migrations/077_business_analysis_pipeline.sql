...
CREATE TABLE agent_work_drafts_next (
  ...
  draft_type TEXT NOT NULL CHECK(draft_type IN (
    ...
    'development', 'verification', 'feedback', 'review', 'business_analysis'
  )),
...
CREATE TABLE business_analysis_drafts (
...
CREATE TABLE business_analysis_phase_artifacts (
  draft_id TEXT NOT NULL REFERENCES business_analysis_drafts(draft_id) ON DELETE CASCADE,
  phase TEXT NOT NULL,
  content TEXT NOT NULL,
  updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY(draft_id, phase)
);

CREATE TABLE business_analysis_phase_transitions (
